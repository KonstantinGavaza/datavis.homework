...
loadData().then(data => {
    ...
    d3.select('#range').on('change', function(){ 
        ...
        yearLable.html(year);
        updateScattePlot(xParam, yParam, rParam);
    ...
    function updateScattePlot(x_axis, y_axis, r_axis){
        ...
        .on('click', function (param_click) {
            ...
            scatterPlot
            .selectAll("circle")
            .style("opacity", d => d.country === choiced_country ? 1.0: .0);
...
async function loadData() {
    var data_url = {
        'child-mortality': 'https://raw.githubusercontent.com/KonstantinGavaza/datavis.homework/main/data/cmu5.csv',
        'gdp': 'https://raw.githubusercontent.com/KonstantinGavaza/datavis.homework/main/data/gdp.csv',
        'fertility-rate': 'https://raw.githubusercontent.com/KonstantinGavaza/datavis.homework/main/data/fertility-rate.csv',
        'life-expectancy': 'https://raw.githubusercontent.com/KonstantinGavaza/datavis.homework/main/data/life_expectancy.csv',
        'population': 'https://raw.githubusercontent.com/KonstantinGavaza/datavis.homework/main/data/population.csv'
        }
    
    const data = { 
        'population': await d3.csv(data_url['population']),
        'gdp': await d3.csv(data_url['gdp']),
        'child-mortality': await d3.csv(data_url['child-mortality']),
        'life-expectancy': await d3.csv(data_url['life-expectancy']),
        'fertility-rate': await d3.csv(data_url['fertility-rate'])
    };
